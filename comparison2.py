...
def bmh_match(text, pattern):
    return boyer_moore_horspool_search(text, pattern)

# Main experiment function
def run_experiment(genome, pattern_lengths):
    patterns = generate_patterns(genome, pattern_lengths)
    results = {algo: [] for algo in ["Naive", "Rabin Single", "Rabin Double", "Improved Rabin", "KMP", "Boyer-Moore", "BMH"]}

    for length in pattern_lengths:
        naive_times, rabin_single_times, rabin_double_times, improved_rabin_times = [], [], [], []
        kmp_times, bm_times, bmh_times = [], [], []
        ...
        for pattern in patterns[length]:
            ...
            kmp_times.append(measure_time(kmp_match, genome, pattern))
            bm_times.append(measure_time(bm_match, genome, pattern))
            bmh_times.append(measure_time(bmh_match, genome, pattern))
        ...
        results["KMP"].append(np.mean(kmp_times))
        results["Boyer-Moore"].append(np.mean(bm_times))
        results["BMH"].append(np.mean(bmh_times))
